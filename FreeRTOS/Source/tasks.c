...
//20 seconds max deadline if tick equals millisecond
#define MAX_REL_DEADLINE 20000
...
typedef struct tskTaskControlBlock
{
	...
	unsigned long ulWCET; //worst case execution time
	unsigned long ulPeriod; //period of task
	unsigned long ulRelativeDeadline;
	unsigned long ulDeadline;
	unsigned long ulStartTime;
	unsigned long ulET; //execution time
...
signed portBASE_TYPE xTaskGenericCreate( pdTASK_CODE pxTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions, unsigned long ulWCET, unsigned long ulPeriod, unsigned long ulRelativeDeadline )
{
...
tskTCB * pxNewTCB;
	...
	{
		//all tasks have same priority!!!! dynamic priorities based on deadline
		uxPriority=LOW_PRIO;
	}
	...
	pxNewTCB->ulWCET = ulWCET; //worst case execution time
	pxNewTCB->ulPeriod = ulPeriod; //period of task
	pxNewTCB->ulRelativeDeadline = ulRelativeDeadline;
	pxNewTCB->ulStartTime = xTickCount;
	pxNewTCB->ulET=0;
	pxNewTCB->ulDeadline = xTickCount+ulRelativeDeadline;
...
portBASE_TYPE xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle,1,1000000,1000000 );
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL,1,1000000,1000000 );
	}
	#endif	
...
tskTCB * pxTCB;
	...
	unsigned long ulMinimalDeadline = xTickCount+MAX_REL_DEADLINE;
	tskTCB* highPrioTCB = NULL;
	xListItem* currentItem= ((xList*)&(pxReadyTasksLists[LOW_PRIO]))->pxIndex;
	int i=0;
	tskTCB* tcbNow;
	for(i=0;i<listCURRENT_LIST_LENGTH(&(pxReadyTasksLists[LOW_PRIO]));i++)
	{	
		listGET_OWNER_OF_NEXT_ENTRY(tcbNow,&(pxReadyTasksLists[LOW_PRIO]));			
			
		if((tcbNow->ulDeadline<=xTickCount)&&(tcbNow->ulET<tcbNow->ulWCET))
		{
			//deadline missed
			uart_putc('y');
			uart_putc('x');
			uart_putl(xTickCount);
			uart_putc('t');
			uart_putc('0'+tcbNow->uxTCBNumber);
			uart_putc('r');
			uart_putl(tcbNow->ulRelativeDeadline);
			uart_putc('d');
			uart_putl(tcbNow->ulDeadline);
			uart_putc('p');
			uart_putl(tcbNow->ulPeriod);
			uart_putc('e');
			uart_putl(tcbNow->ulET);
			uart_putc('w');
			uart_putl(tcbNow->ulWCET);
			uart_putc('i');
			uart_putc('0'+tcbNow->uxPriority);
			uart_putc('\n');
		}

	//	if (tickcount-t.Starttime) % t.period == 0:
		if(((xTickCount-tcbNow->ulStartTime) % tcbNow->ulPeriod)==0)
		{
			//t.Deadline = tickcount+t.relativeDeadline
			tcbNow->ulDeadline = xTickCount+tcbNow->ulRelativeDeadline;
			tcbNow->ulET=0;
		}
	//	if mindealine > t.Deadline && t.ET<t.WCET:
		if(tcbNow->ulET<tcbNow->ulWCET && ulMinimalDeadline>tcbNow->ulDeadline)
		{
			//t wird neuer_active_task(t)
			ulMinimalDeadline = tcbNow->ulDeadline;
			highPrioTCB = tcbNow;
		}
	}
	...
	//für current TCB
		...
		if(((pxCurrentTCB->ulDeadline)<=(xTickCount))&&((pxCurrentTCB->ulET)<(pxCurrentTCB->ulWCET)))
		{
			//deadline missed
			uart_putc('!');
			uart_putc('x');
			uart_putl(xTickCount);
			uart_putc('t');
			uart_putc('0'+pxCurrentTCB->uxTCBNumber);
			uart_putc('r');
			uart_putl(pxCurrentTCB->ulRelativeDeadline);
			uart_putc('d');
			uart_putl(pxCurrentTCB->ulDeadline);
			uart_putc('p');
			uart_putl(pxCurrentTCB->ulPeriod);
			uart_putc('e');
			uart_putl(pxCurrentTCB->ulET);
			uart_putc('w');
			uart_putl(pxCurrentTCB->ulWCET);
			uart_putc('i');
			uart_putc('0'+pxCurrentTCB->uxPriority);
			uart_putc('\n');
		}

	//	if (tickcount-t.Starttime) % t.period == 0:
		if(((xTickCount-pxCurrentTCB->ulStartTime) % pxCurrentTCB->ulPeriod)==0)
		{
			//t.Deadline = tickcount+t.relativeDeadline
			pxCurrentTCB->ulDeadline = xTickCount+pxCurrentTCB->ulRelativeDeadline;
			pxCurrentTCB->ulET=0;
		}
	//	if mindealine > t.Deadline && t.ET<t.WCET:
		if(!(pxCurrentTCB->ulET<pxCurrentTCB->ulWCET && ulMinimalDeadline>pxCurrentTCB->ulDeadline))
		{
			//vTaskPrioritySetScheduler(pxCurrentTCB,LOW_PRIO); //lowest Prio
			pxCurrentTCB->uxPriority = LOW_PRIO;
			listSET_LIST_ITEM_VALUE( &( pxCurrentTCB->xEventListItem ), ( configMAX_PRIORITIES - LOW_PRIO ) );
			vListRemove( &( pxCurrentTCB->xGenericListItem ) );
			prvAddTaskToReadyQueue( pxCurrentTCB );


			//vTaskPrioritySetScheduler(highPrioTCB,HIGH_PRIO); //highest prio
			highPrioTCB->uxPriority = HIGH_PRIO;
			listSET_LIST_ITEM_VALUE( &( highPrioTCB->xEventListItem ), ( configMAX_PRIORITIES - HIGH_PRIO ) );
			vListRemove( &( highPrioTCB->xGenericListItem ) );
			prvAddTaskToReadyQueue( highPrioTCB );
		}
